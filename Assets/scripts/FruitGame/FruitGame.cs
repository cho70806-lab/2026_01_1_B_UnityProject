...
public class FruitGame : MonoBehaviour
{
    ...
    public float fruitStertHeight = 6.0f;
    ...
    void Update()
    {
        ...
        if(currentFruit != null)
        {
            ...
            Vector3 worldPosition = mainCarmera.ScreenToWorldPoint(mousePosition);
    ...
    void SpawnNewFruit()
    {
        if(!isGameOver)
        {
            ...
            Vector3 spawnPosition = new Vector3(worldPosition.x, fruitStertHeight, -0);

            float halfFruitSize = fruitSizes[currentFruitType] / 2f;

            spawnPosition.x = Mathf.Clamp(spawnPosition.x, -gameWidth / 2 + halfFruitSize, gameWidth / -halfFruitSize);
